...
export class BadgeRenderer {
    ...
    renderBadgeTooltip(badge, options) {
        const themeStyle = this.themeStyles.get(options.theme);
        if (!themeStyle) {
            throw new Error(`Unknown theme: ${options.theme}`);
        }
    ...
    renderBadgeOverlay(badges, options) {
        const themeStyle = this.themeStyles.get(options.theme);
        if (!themeStyle) {
            throw new Error(`Unknown theme: ${options.theme}`);
        }
    ...
    renderContributorBadges(contributorId, badges, options) {
        const themeStyle = this.themeStyles.get(options.theme);
        if (!themeStyle) {
            throw new Error(`Unknown theme: ${options.theme}`);
        }
    ...
    renderCreditsOverlay(contributors, options) {
        const themeStyle = this.themeStyles.get(options.theme);
        if (!themeStyle) {
            throw new Error(`Unknown theme: ${options.theme}`);
        }
        const contributorList = Array.from(contributors.entries()).map(([contributorId, badges]) => ({
            contributorId,
            badgeCount: badges.length,
            topBadge: badges.reduce((top, current) => this.getBadgeScore(current) > this.getBadgeScore(top) ? current : top),
            recentBadge: badges.reduce((recent, current) => new Date(current.timestamp) > new Date(recent.timestamp) ? current : recent)
        }));
        return {
...
//# sourceMappingURL=renderBadges.js.map