...
import { BadgeRenderer } from './renderBadges';
export class CreditsRenderer {
    ...
    // Section Creation
    createCreditsSections(allBadges, contributors, options) {
        const sections = [];
        // Core Framework Section
        const coreBadges = allBadges.filter(badge => badge.sourceZone === 'miff_framework');
        ...
        const zoneBadges = allBadges.filter(badge => badge.sourceZone === 'overlink_pure');
        ...
        const themeBadges = allBadges.filter(badge => ['neon_grid', 'forest_glade', 'cosmic_void'].includes(badge.sourceZone));
        ...
        const audioBadges = allBadges.filter(badge => badge.sourceZone === 'audio_manager');
        ...
        const lineageBadges = allBadges.filter(badge => badge.sourceZone === 'remix_lab');
    ...
    createContributorCredits(badges, contributors, options) {
    ...
    getContributorName(contributorId) {
        const nameMap = {
    ...
    getContributorRole(contributorId) {
        const roleMap = {
    ...
    // Badge Integration
    renderCreditsWithBadges(options) {
        ...
        const renderedSections = credits.sections.map(section => ({
            ...
            contributors: section.contributors.map(contributor => ({
                ...contributor,
                renderedBadges: this.badgeRenderer.renderContributorBadges(contributor.id, contributor.badges, badgeOptions)
            })),
            ...
            sections: credits.sections.map(section => ({
                ...
                contributors: section.contributors.map(contributor => ({
                    ...
                    badgeSummary: {
                        ...
                        topLevel: contributor.badges.reduce((top, current) => this.getBadgeLevelScore(current.metadata.badgeLevel) > this.getBadgeLevelScore(top.metadata.badgeLevel) ? current : top).metadata.badgeLevel
                    }
                })),
    ...
    renderBadgeFocusedCredits(options) {
        const credits = this.generateCredits(options);
        const badgeOptions = {
            theme: options.theme,
            size: 'large',
        ...
        const contributorBadges = new Map();
    ...
    // Utility Functions
    getTopContributors(sections) {
        const contributorScores = new Map();
        sections.forEach(section => {
            section.contributors.forEach(contributor => {
                const score = contributor.badges.reduce((total, badge) => total + this.getBadgeLevelScore(badge.metadata.badgeLevel), 0);
...
//# sourceMappingURL=renderCredits.js.map