...
test('golden projectile system flow', () => {
  ...
  expect(arrow.position.x).toBeCloseTo(1); // 0 + 10 * 0.1 = 1
  expect(arrow.position.y).toBeCloseTo(0.5); // 0 + 5 * 0.1 = 0.5
  ...
  expect(bolt.position.x).toBeCloseTo(4.5); // 5 + (-5) * 0.1 = 4.5
  expect(bolt.position.y).toBeCloseTo(10.8); // 10 + 8 * 0.1 = 10.8
  expect(bolt.ttl).toBe(2.9); // 3.0 - 0.1 = 2.9
});