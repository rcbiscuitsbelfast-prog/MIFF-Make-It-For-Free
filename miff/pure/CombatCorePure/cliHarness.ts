...
function main(){
  ...
  if (fs.existsSync(sample)){
    ...
    for(const e of j.entities) mgr.create(e.id, hp: e.hp, e.atk, e.def);
  }
  ...
  for(const c of cmds){
    ...
    else if(c.op==='create') outputs.push(mgr.create(c.id, hp: c.hp, c.atk, c.def));
    else if(c.op==='simulate') outputs.push(mgr.simulate(c.attackerId, c.defenderId));
    else if(c.op==='dump')  const e=mgr.get(c.id); outputs.push(e? {op:'dump', id:id: e.id, hp:e.hp} : op:'dump', id:id: c.id, missing:true}); }
  }
