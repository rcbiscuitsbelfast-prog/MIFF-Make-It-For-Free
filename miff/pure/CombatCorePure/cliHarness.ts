...
function main(...args: any[]) {
  ...
  if (fs.existsSync(sample)){
    const j = SafeJSONParser.parse(fs.readFileSync(path.resolve(sample), 'utf-8')) as {entities:{id:string;hp:number;atk:number;def:number}[]};
    for(const e of j.entities) mgr.create(e.id, e.hp, e.atk, e.def);
  }
  const cmds:Cmd[] = commands? SafeJSONParser.parse(fs.readFileSync(path.resolve(commands), 'utf-8')) : [{op:'list'} as Cmd];
