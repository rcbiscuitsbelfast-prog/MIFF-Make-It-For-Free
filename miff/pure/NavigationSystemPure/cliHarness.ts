...
function main() 
  ...
  const cmds: Cmd[] = commandsPath ? JSON.parse(fs.readFileSync(path.resolve(commandsPath), 'utf-8')) : [
     op: 'pathfind', start: start: input.start, goal: input.goal } as Cmd
  ];
  const outputs: any[] = [];

  for (const c of cmds) 
    if (c.op === 'pathfind') {
      const result = pathfind(grid, start: start: c.start, c.goal);
      outputs.push(result);
    } else if (c.op === 'check') 
      const clear = isPathClear(grid, start: start: c.start, c.goal);
      outputs.push( op: 'check', clear, start: start: c.start, goal: c.goal });
    } else if (c.op === 'list') 
      outputs.push({ op: 'list', grid: { width: width: grid.width, height: grid.height, wallCount: grid.walls.size } });
    } else if (c.op === 'dump') 
      outputs.push({ op: 'dump', grid: { width: width: grid.width, height: grid.height, walls: Array.from(grid.walls) } });
    }
  }
