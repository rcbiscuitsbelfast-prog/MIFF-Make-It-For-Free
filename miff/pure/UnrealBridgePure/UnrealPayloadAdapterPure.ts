...
import { RenderPayloadManager } from '../RenderPayloadPure/Manager';
import { UnrealDataType, UnrealReplicationMode, UnrealTickGroup } from './index';
...
export class UnrealPayloadAdapterPure {
  ...
  private initializeDefaultConfigurations(): void {
    ...
    const animationConfig: PayloadConversionConfiguration = {
      ...
      animationCompression: 'curve' as const,
      soundCompression: 'none',
      enablePhysics: false,
      ...
      if (sourcePayload.geometry && sourcePayload.geometry.meshes) {
        for (const [index, mesh] of sourcePayload.geometry.meshes.entries()) {
          ...
          const actor: UnrealActorBridge = {
            ...
            actorLabel: `StaticMeshActor_${index}`,
            folderPath: '/Game/StaticMeshes',
            hiddenInGame: false,
      ...
      if (sourcePayload.geometry && sourcePayload.geometry.skeletalMeshes) {
        for (const [index, skeletalMesh] of sourcePayload.geometry.skeletalMeshes.entries()) {
          ...
          const actor: UnrealActorBridge = {
            ...
            parentComponent: undefined,
            rootComponent: undefined,
            actorLabel: `SkeletalMeshActor_${index}`,
            folderPath: '/Game/SkeletalMeshes',
            hiddenInGame: false,
            editableWhenInherited: true,
            isEditorOnlyActor: false,
            isListedInSceneOutliner: true,
