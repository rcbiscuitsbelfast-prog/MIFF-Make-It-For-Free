...
export class UnrealEventSyncPure {
  ...
  private async initializeEventSync(): Promise<void> {
    console.info('[UnrealEventSyncPure] Initializing event synchronization...');
    ...
    try {
      ...
      console.info('[UnrealEventSyncPure] Event synchronization initialized successfully');
    } catch (error) {
  ...
  private async initializeEventQueues(): Promise<void> {
    console.info('[UnrealEventSyncPure] Initializing event queues...');
    ...
    console.info(`[UnrealEventSyncPure] Initialized ${this.eventQueues.size} event queues`);
  }

  private async initializeEventMappings(): Promise<void> {
    console.info('[UnrealEventSyncPure] Initializing event mappings...');
    ...
    console.info(`[UnrealEventSyncPure] Initialized ${this.eventMappings.size} event mappings`);
  }

  private async initializeEventFilters(): Promise<void> {
    console.info('[UnrealEventSyncPure] Initializing event filters...');

    if (!this.configuration?.enableEventFiltering) {
      console.info('[UnrealEventSyncPure] Event filtering disabled');
      return;
    }
    ...
    console.info(`[UnrealEventSyncPure] Initialized ${this.eventFilters.size} event filters`);
  }

  private async initializeEventTransformers(): Promise<void> {
    console.info('[UnrealEventSyncPure] Initializing event transformers...');
    ...
    console.info(`[UnrealEventSyncPure] Initialized ${this.eventTransformers.size} event transformers`);
  }

  private async initializeEventValidators(): Promise<void> {
    console.info('[UnrealEventSyncPure] Initializing event validators...');
    ...
    console.info(`[UnrealEventSyncPure] Initialized ${this.eventValidators.size} event validators`);
  }
  ...
  private startEventProcessing(): void {
    ...
    console.info('[UnrealEventSyncPure] Event processing started');
  }
  ...
  async syncEvent(miffEvent: any): Promise<boolean> {
    ...
    console.info(`[UnrealEventSyncPure] Syncing MIFF event: ${miffEvent.type || miffEvent.name}`);
    ...
    try {
      ...
      if (this.configuration.enableEventFiltering) {
        ...
        if (!filtered.allowed) {
          console.info(`[UnrealEventSyncPure] Event filtered: ${unrealEvent.name} (${filtered.reason})`);
      ...
      console.info(`[UnrealEventSyncPure] Event synced successfully: ${transformedEvent.name} (${processingTime}ms)`);
      return true;
  ...
  private async convertMiffEventToUnreal(miffEvent: any): Promise<UnrealEvent | null> {
    ...
    if (!mapping.enabled) {
      console.info(`[UnrealEventSyncPure] Event mapping disabled: ${mapping.miffEvent}`);
      return null;
    }
  ...
  private async processEventBuffer(): Promise<void> {
    ...
    console.info(`[UnrealEventSyncPure] Processing event buffer: ${this.eventBuffer.length} events`);
  ...
  subscribeToEvent(subscription: EventSubscription): void {
    ...
    console.info(`[UnrealEventSyncPure] Event subscription created: ${subscription.id} for ${subscription.eventType}`);
  }

  unsubscribeFromEvent(subscriptionId: string): void {
    this.eventSubscriptions.delete(subscriptionId);
    console.info(`[UnrealEventSyncPure] Event subscription removed: ${subscriptionId}`);
  }

  getEventSubscription(subscriptionId: string): EventSubscription! {
  ...
  addEventMapping(mapping: EventMapping): void {
    this.eventMappings.set(mapping.miffEvent, mapping);
    console.info(`[UnrealEventSyncPure] Event mapping added: ${mapping.miffEvent} → ${mapping.unrealEvent}`);
  }

  removeEventMapping(miffEvent: string): void {
    this.eventMappings.delete(miffEvent);
    console.info(`[UnrealEventSyncPure] Event mapping removed: ${miffEvent}`);
  }

  getEventMapping(miffEvent: string): EventMapping! {
  ...
  updateConfiguration(updates: Partial<EventSyncConfiguration>): void {
    ...
    console.info('[UnrealEventSyncPure] Configuration updated');
  }
  ...
  clearEventBuffer(): void {
    this.eventBuffer = [];
    console.info('[UnrealEventSyncPure] Event buffer cleared');
  }

  clearDeadLetterQueue(): void {
    this.deadLetterQueue = [];
    console.info('[UnrealEventSyncPure] Dead letter queue cleared');
  }
  ...
  clearAllQueues(): void {
    ...
    console.info('[UnrealEventSyncPure] All event queues cleared');
  }
  ...
  reset(): void {
    ...
    console.info('[UnrealEventSyncPure] Event synchronization reset to initial state');
  }

  dispose(): void {
    this.reset();
    this.isInitialized = false;
    console.info('[UnrealEventSyncPure] Event synchronization disposed successfully');
  }
}