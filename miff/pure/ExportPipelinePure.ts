...
this.logger.info('Game loaded: ${config.projectName}');
this.logger.info('Engine: ${config.engine}');
this.logger.info('Platform: ${config.platform}');
...
`;
  ...
  private log(level: string, message: string): void {
    if (this.config.logLevel === 'debug' || (this.config.logLevel === 'info' && level !== 'debug')) {
      this.logger.info(`[ExportPipeline] ${level.toUpperCase()}: ${message}`);
    }
  }
