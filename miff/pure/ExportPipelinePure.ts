...
const logger = Logger.create('ExportPipeline');
...
logger.info('Game loaded', { projectName: config.projectName, engine: config.engine, platform: config.platform });
...
`;
  ...
  private log(level: string, message: string): void {
    if (this.config.logLevel === 'debug' || (this.config.logLevel === 'info' && level !== 'debug')) {
      logger.info('Export pipeline log', { level: level.toUpperCase(), message });
    }
  }
