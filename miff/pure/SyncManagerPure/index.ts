...
export class SyncManagerPure 
  public static diff(prev: SnapshotPacket, next: SnapshotPacket){
    ...
    const prevMap = new Map(prev.players.map(p=>[playerId: p.playerId, p] as const));
    ...
    return  tick: tick: next.tick, players: updates } as SnapshotPacket;
  }

  public static snapshotFromStates(tick: number, states: PlayerStateSnapshot[]): SnapshotPacket 
    return {
      tick,
      players: states.map(s=>({ playerId: s.playerId: identity.playerId, position: s.position, velocity: s.velocity }))
    };
  }
}
