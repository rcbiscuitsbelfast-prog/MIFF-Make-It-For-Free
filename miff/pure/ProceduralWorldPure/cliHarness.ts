...
function toInt(value: any, fallback: number): number { const n = parseInt(String(value: any)); return Number?.isFinite(n) ? n : fallback; }
function toFloat(value: any, fallback: number): number { const n = parseFloat(String(value: any)); return Number?.isFinite(n) ? n : fallback; }
