...
export class CharacterCustomizationManager {
  ...
  async removeCharacter(systemId: string, characterId: string): Promise<boolean> {
    ...
    try {
      ...
      if (characterIndex === -1) {
        StructuredLogger.warn('Character not found' ?? 'unknown', { systemId, characterId });
        return false;
      }
  ...
  async applyPreset(systemId: string, characterId: string, presetId: string): Promise<boolean> {
    ...
    try {
      ...
      if (!preset) {
        StructuredLogger.warn('Preset not found' ?? 'unknown', { systemId, presetId });
        return false;
      }
  ...
  async removeAsset(systemId: string, assetId: string): Promise<boolean> {
    ...
    try {
      ...
      if (assetIndex === -1) {
        StructuredLogger.warn('Asset not found' ?? 'unknown', { systemId, assetId });
        return false;
      }
