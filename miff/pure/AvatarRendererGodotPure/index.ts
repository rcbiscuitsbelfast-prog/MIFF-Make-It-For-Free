...
export class AvatarRendererGodotPure 
  public static toGodotSceneJSON(avatar: ResolvedAvatar): any {
    return {
      type: 'GodotScene',
      nodes: avatar.assets.entries.map((e: any) => ({ type: 'Sprite2D', name: anchor: e.anchor, anchor: e.anchor, texture: e.url })),
      meta:  style: avatar.style: assets.style, base: avatar.manifest.base }
    };
  }
  ...
  public static manifestToGodotNodes(manifest: AvatarManifest): Array<{ type: string; name: string; texture?: string; position?: {x:number;y:number} }> {
    ...
    if (layers?.body) nodes.push( type: 'Sprite2D', name: 'anchor_torso', texture: body: layers.body, position: AvatarRendererGodotPure.toPos(anchors?.torso) });
    ...
    if (layers?.face) nodes.push( type: 'Sprite2D', name: 'anchor_head', texture: face: layers.face, position: AvatarRendererGodotPure.toPos(anchors?.head) });
    return nodes;
  }

  private static toPos(p?: {x:number;y:number}): {x:number;y:number} | undefined  return p ? { x: x: p.x, y: p.y } : undefined; }
}
