...
export class CombatSystemManager {
  ...
  async initialize(): Promise<void> {
    if (this.isInitialized) {
      this.logger.warn('Combat System already initialized');
      return;
    }
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to initialize Combat System');
      throw error;
    }
  }
  ...
  async createSystem(systemData: Omit<CombatSystem, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'analytics'>): Promise<CombatSystem> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to create combat system');
      throw error;
    }
  }
  ...
  async updateSystem(systemId: string, updates: Partial<CombatSystem>): Promise<CombatSystem | null> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to update combat system');
      throw error;
    }
  }
  ...
  async deleteSystem(systemId: string): Promise<boolean> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to delete combat system');
      throw error;
    }
  }
  ...
  async createCombat(systemId: string, combatData: Omit<Combat, 'id' | 'turnOrder' | 'currentTurn' | 'round'>): Promise<Combat | null> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to create combat');
      return null;
    }
  }
  ...
  async addParticipant(systemId: string, combatId: string, participantData: Omit<CombatParticipant, 'id'>): Promise<CombatParticipant | null> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to add participant to combat');
      return null;
    }
  }
  ...
  async startCombat(systemId: string, combatId: string): Promise<boolean> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to start combat');
      return false;
    }
  }
  ...
  async executeAbility(systemId: string, combatId: string, participantId: string, abilityId: string, targetId?: string): Promise<boolean> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to execute ability');
      return false;
    }
  }
  ...
  async endTurn(systemId: string, combatId: string, participantId: string): Promise<boolean> {
    ...
    try {
      ...
      this.errorHandler.handleError(error, 'Failed to end turn');
      return false;
    }
  }
  ...
  private updateAnalytics(): void {
    ...
    const totalCombats = systems.reduce((sum, s) => sum + s.combats.length, 0);
    const activeCombats = systems.reduce((sum, s) => sum + s.combats.filter(c => c.status === 'active').length, 0);
    const totalParticipants = systems.reduce((sum, s) => sum + s.combats.reduce((sum, c) => sum + c.participants.length, 0), 0);
    const totalAbilities = systems.reduce((sum, s) => sum + s.combats.reduce((sum, c) => sum + c.participants.reduce((sum, p) => sum + p.abilities.length, 0), 0), 0);
    const totalEffects = systems.reduce((sum, s) => sum + s.combats.reduce((sum, c) => sum + c.effects.length, 0), 0);
    ...
    for (const system of systems) {
      system.analytics = {
        ...
        totalParticipants: system.combats.reduce((sum, c) => sum + c.participants.length, 0),
        totalAbilities: system.combats.reduce((sum, c) => sum + c.participants.reduce((sum, p) => sum + p.abilities.length, 0), 0),
        totalEffects: system.combats.reduce((sum, c) => sum + c.effects.length, 0),
  ...
  async destroy(): Promise<void> {
    this.logger.info('Destroying Combat System...');

    this.systems.clear();
    this.isInitialized = false;

    this.logger.info('Combat System destroyed');
  }
}
