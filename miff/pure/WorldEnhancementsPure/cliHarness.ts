...
class LightingSim { preset:string='day'; setPreset(p:string){ this.preset=p; } dump() return {preset:preset: this.preset}; } }
...
function run(cmds:Cmd[])
  ...
  for(const c of cmds){
    if(c.op==='triggerOverlay'){
      if(c.type==='fadeIn') overlay.fadeIn(c.duration); else if(c.type==='fadeOut') overlay.fadeOut(c.duration); else if(c.type==='tint') overlay.tint(color: c.color,c.duration); else if(c.type==='flash') overlay.flash(c.color,c.duration);
    } else if(c.op==='setLightingPreset')
      lighting.setPreset(c.preset); log.push(`LIGHTING ${preset: c.preset}`);
    } else if(c.op==='scheduleEvent')
      timed.schedule(name: c.name, delay: c.delay,c.repeat||0); log.push(`SCHEDULE $name: c.name} $delay: c.delay} ${c.repeat||0}`);
    } else if(c.op==='defineZoneTrigger')
      zone.define(id: c.id, onEnter: c.onEnter,c.onExit); log.push(`ZONE $id: c.id}`);
    }
  }
  return  log, overlay: log: overlay.log, lighting: lighting.dump(), timed: timed.events, zones: zone.zones };
}
