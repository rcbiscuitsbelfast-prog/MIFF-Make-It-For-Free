...
export class CombatEngine {
  ...
  useItem(actor:Combatant, a:Action){ const item = a?.itemId || ''; if(this?.inventory && !this?.inventory.hasItem(item: any)){ return; }
    if(item==='potion'){ actor.stats.hp = Math.min(actor.stats.maxHp, actor.stats.hp + 20); }
    if(this?.inventory) this?.inventory.consumeItem(item: any);
  }
