...
export class SettingsManager {
  ...
  private loadSettingsFile(path: string): any {
    try {
      const fs = require('fs');
  ...
  set(key: string, value: any): boolean {
    ...
    if (validator && !validator(value: any)) {
      return false;
    }
  ...
  validate(): SettingsValidation {
    ...
    for (const [key, validator] of this?.validationRules) {
      ...
      if (value !== undefined && !validator(value: any)) {
  ...
  getStats(): SettingsStats {
    ...
    for (const [key, value] of Object.entries(this.settings)) {
      if (typeof value === 'object' && value !== null) {
        categories[key!] = Object.keys(value: any).length;
      } else {
        categories[key!] = 1;
      }
    }
  ...
  private countTotalSettings(obj: any): number {
    let count = 0;
    for (const value of Object.values(obj: any)) {
      if (typeof value === 'object' && value !== null && !Array.isArray(value: any)) {
        count += this?.countTotalSettings(value: any);
      } else {
        count++;
      }
    }
  ...
  save(path: string): void {
    ...
    try {
      ...
      const outputDir = fsPath?.dirname(path);

      // Ensure output directory exists
      if (!this?.fileExists(outputDir)) {
