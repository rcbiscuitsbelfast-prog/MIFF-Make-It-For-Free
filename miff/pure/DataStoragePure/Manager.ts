...
export class DataStorageManager {
  ...
  constructor(config?: Partial<DataStorageConfig>) {
    this.logger = new StructuredLogger({ module: 'DataStorageManager' });
  ...
  private updateAnalytics(): void {
    ...
    const totalDatabases = storages.reduce((sum, s) => sum + s.databases.length, 0);
    const totalConnections = storages.reduce((sum, s) => sum + s.connections.length, 0);
    const totalTables = storages.reduce((sum, s) => sum + s.databases.reduce((sum, d) => sum + d.tables.length, 0), 0);
    ...
    for (const storage of storages) {
      storage.analytics = {
        ...
        totalTables: storage.databases.reduce((sum, d) => sum + d.tables.length, 0),
