...
export class DataStorageManager {
  ...
  async removeDatabase(storageId: string, databaseId: string): Promise<boolean> {
    ...
    try {
      ...
      if (databaseIndex === -1) {
        StructuredLogger.warn('Database not found' ?? 'unknown', { storageId, databaseId });
        return false;
      }
  ...
  async removeConnection(storageId: string, connectionId: string): Promise<boolean> {
    ...
    try {
      ...
      if (connectionIndex === -1) {
        StructuredLogger.warn('Connection not found' ?? 'unknown', { storageId, connectionId });
        return false;
      }
  ...
  async dropTable(storageId: string, databaseId: string, tableId: string): Promise<boolean> {
    ...
    try {
      ...
      if (tableIndex === -1) {
        StructuredLogger.warn('Table not found' ?? 'unknown', { storageId, databaseId, tableId });
        return false;
      }
