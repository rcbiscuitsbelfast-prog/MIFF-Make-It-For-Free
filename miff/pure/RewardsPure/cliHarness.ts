...
function printStatus(state: CLIState): void {
  ...
  console.info(`Reward History: ${state.rewardHistory.length} rewards`);
  console.info(`Drop Table: ${state.currentDropTable ? state.currentDropTable.entries.length + ' items' : 'None'}`);
  if (state.currentDropTable && state.currentDropTable.entries.length > 0) {
    console.info(`Total Weight: ${state.currentDropTable.getTotalWeight()}`);
  }
  console.info(`Simulations Run: ${state.simulationResults.size > 0 ? 'Yes' : 'None'}`);
}

function printTable(dropTable: DropTable | null): void {
  if (!dropTable || dropTable.entries.length === 0) {
    console.info('❌ No drop table loaded or table is empty');
    return;
  }

  console.info('\n🎲 Current Drop Table:');
  console.info(`Total Entries: ${dropTable.entries.length}`);
  console.info(`Total Weight: ${dropTable.getTotalWeight()}`);
  ...
  if (dropTable.entries.length > 0) {
    ...
    dropTable.getEntriesByWeight().forEach((entry, index) => {
      ...
      console.info(`  ${index + 1}. ${entry.itemId} (weight: ${entry.weight}, rate: ${dropRate}%)`);
    });
  }
}
...
function printHistory(rewardHistory: RewardStub[]): void {
  if (rewardHistory.length === 0) {
    console.info('📜 No rewards in history');
    return;
  }

  console.info('\n📜 Reward History:');
  rewardHistory.slice(-10).forEach((reward, index) => {
    console.info(`  ${rewardHistory.length - 10 + index + 1}. ${reward.toString()}`);
  });

  if (rewardHistory.length > 10) {
    console.info(`  ... and ${rewardHistory.length - 10} more`);
  }
...
function createDemoData(): { rewardManager: RewardManager; rng: IRNGProvider; dropTable: DropTable } {
  console.info('🎮 Creating demo reward system...');
  ...
  console.info('✅ Demo data created with reward manager and drop table');
  return { rewardManager, rng, dropTable };
}

function runDemo(state: CLIState): void {
  console.info('🎯 Running RewardsPure Demo...\n');

  // Test reward generation at different levels
  console.info('--- Reward Generation Test ---');
  const scenarios = [
  ...
  scenarios.forEach(scenario => {
    ...
    console.info(`${scenario.type} (P${scenario.playerLevel} vs E${scenario.enemyLevel}): ${reward.toString()}`);
  ...
  bonusTypes.forEach(type => {
    ...
    console.info(`${type} bonus: ${bonusReward.toString()}`);
  });
...
function runSimulation(state: CLIState, runs: number): void {
  console.info(`🎲 Running ${runs} reward simulations...\n`);
  ...
  console.info(`✅ Simulation complete in ${duration}ms`);
  console.info(`Generated ${runs} rewards`);
  console.info(`Total currency: ${state.rewardHistory.slice(-runs).reduce((sum, r) => sum + r.currency, 0)}`);
  console.info(`Total XP: ${state.rewardHistory.slice(-runs).reduce((sum, r) => sum + r.xpGain, 0)}`);
  console.info(`Items received: ${state.rewardHistory.slice(-runs).filter(r => r.itemId).length}`);
}
...
async function runCLI(): Promise<void> {
  ...
  console.info('🎁 RewardsPure CLI - Type "help" for commands or "demo" to see rewards in action\n');
  ...
  rl.on('line', (input: string) => {
    ...
    switch (command) {
      ...
      case 'reward':
        if (args.length < 2) {
          console.info('❌ Usage: reward <encounter_type> <player_level> <enemy_level>');
        } else {
          ...
          if (isNaN(playerLevel) || isNaN(enemyLevel)) {
            console.info('❌ Player and enemy levels must be numbers');
          } else {
            const reward = state.rewardManager.generateRewards(encounterType, playerLevel, enemyLevel);
            state.rewardHistory.push(reward);
            console.info(`✅ Generated reward: ${reward.toString()}`);
          }
        }
        break;
      ...
      case 'add':
        if (args.length < 2) {
          console.info('❌ Usage: add <item_id> <weight>');
        } else {
          const itemId = args[0];
          const weight = parseFloat(args[1]);

          if (isNaN(weight) || weight < 0) {
            console.info('❌ Weight must be a non-negative number');
          } else {
            ...
            if (state.currentDropTable.addEntry(entry)) {
              console.info(`✅ Added ${itemId} to drop table (weight: ${weight})`);
            } else {
              console.info('❌ Failed to add item');
            }
          }
        }
        break;
      ...
      case 'remove':
        if (!args[0]) {
          console.info('❌ Usage: remove <item_id>');
        } else {
          if (state.currentDropTable) {
            const removed = state.currentDropTable.removeEntriesByItem(args[0]);
            console.info(`✅ Removed ${removed} entries for item: ${args[0]}`);
          } else {
            console.info('❌ No drop table loaded');
          }
        }
        break;
      ...
      case 'drop':
        if (args.length === 0) {
          console.info('❌ Usage: drop <count>');
        } else {
          const count = parseInt(args[0]);

          if (isNaN(count) || count <= 0) {
            console.info('❌ Count must be a positive number');
          } else if (!state.currentDropTable || state.currentDropTable.entries.length === 0) {
            console.info('❌ No drop table loaded');
          } else {
            console.info(`🎲 Simulating ${count} drops...\n`);
      ...
      case 'simulate':
        if (args.length === 0) {
          console.info('❌ Usage: simulate <runs>');
        } else {
          const runs = parseInt(args[0]);

          if (isNaN(runs) || runs <= 0) {
            console.info('❌ Runs must be a positive number');
          } else {
      ...
      case 'config':
        if (args.length === 0) {
          ...
          console.info('Current reward scaling configuration:');
          console.info(`  Base Currency: ${config.baseCurrency}`);
          console.info(`  Level Currency Multiplier: ${config.levelCurrencyMultiplier}`);
          console.info(`  Base XP: ${config.baseXP}`);
          console.info(`  Level XP Multiplier: ${config.levelXPMultiplier}`);
          ...
          if ([baseCurrency, levelCurrencyMultiplier, baseXP, levelXPMultiplier].some(isNaN)) {
            console.info('❌ All values must be numbers');
          } else {
            state.rewardManager.configureScaling(baseCurrency, levelCurrencyMultiplier, baseXP, levelXPMultiplier);
            console.info('✅ Reward scaling configuration updated');
          }
        } else {
          console.info('❌ Usage: config [base_currency level_currency_mult base_xp level_xp_mult]');
        }
        break;
