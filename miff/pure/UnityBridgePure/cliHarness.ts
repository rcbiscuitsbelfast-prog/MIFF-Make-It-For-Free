...
import { UnityBridge, UnityBridgeOperation, UnityBridgeOutput, UnityBridgeConfig } from './Bridge';

const bridge = new UnityBridge();

function printUsage(): void {
  console.log(`
UnityBridgePure CLI Harness - Schema v1

Usage: npx ts-node UnityBridgePure/cliHarness.ts <command> [args...]

Commands:
  simulate <module> <dataFile> [configFile] - Simulate module with Unity bridge
  render <module> <dataFile> [configFile]   - Render module for Unity
  interop <module> <dataFile> [configFile]  - Interop with Unity data
  dump <module>                             - Dump module bridge info

Modules:
  npcs, combat, crafting, loot, economy, quests, stats, world
...
function loadConfig(configFile?: string): UnityBridgeConfig {
  const defaultConfig: UnityBridgeConfig = {
...
function main(): void {
  ...
  try {
    ...
    const errorResult: UnityBridgeOutput = {
      op: command,
      status: 'error',
