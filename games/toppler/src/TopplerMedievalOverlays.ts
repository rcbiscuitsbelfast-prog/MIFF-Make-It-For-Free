...
export class TopplerMedievalOverlays {
    ...
    private overlayTransitions: Map<string, { resolve: () => void; reject: () => void }> = new Map();
    ...
    private sprites = {
        knight: null as HTMLImageElement | null,
        skeleton: null as HTMLImageElement | null,
        isle: null as HTMLImageElement | null,
        chest: null as HTMLImageElement | null,
        tileBackground: null as HTMLImageElement | null
    };
    ...
    private initializeOverlayStyles(): void {
    ...
    private async triggerOverlay(trigger: string): Promise<void> {
        ...
        for (const [transitionId, transition] of Object.entries(this.orchestration.orchestration.transitions)) {
            if (trigger === transitionId) {
                if (transition.from === '*' || transition.from === this.state.currentScene) {
                    this.state.currentScene = transition.to as any;
                    if (transition.modal) {
    ...
    private async showOverlay(overlayId: string, config: OverlayConfig): Promise<void> {
    ...
    private async hideOverlay(overlayId: string): Promise<void> {
    ...
    private createOverlayContent(overlay: HTMLElement, overlayId: string, config: OverlayConfig): void {
        switch (overlayId) {
            case 'TopplerIntro':
    ...
    private createIntroContent(overlay: HTMLElement): void {
    ...
    private createGameOverContent(overlay: HTMLElement): void {
        const title = document.createElement('div');
        title.className = 'overlay-title';
    ...
    private createLoreContent(overlay: HTMLElement): void {
        const title = document.createElement('div');
        title.className = 'overlay-title';
    ...
    private createHudContent(overlay: HTMLElement): void {
    ...
    private showOptionsMenu(): void {
        ...
        difficultySelect.onchange = () => {
            this.state.difficulty = difficultySelect.value as any;
            this.persistState();
        };
    ...
    private async handleOverlayAction(action: string): Promise<void> {
    ...
    private updateInputModeDisplay(): void {
    ...
    private async triggerGameOver(reason: string): Promise<void> {
        this.state.gameActive = false;
