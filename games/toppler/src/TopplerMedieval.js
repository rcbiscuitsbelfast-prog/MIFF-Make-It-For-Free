...
export class TopplerMedieval {
    ...
    updateGamepad() {
        if (this.gamepadIndex === -1 || this.state.inputMode !== 'gamepad')
            return;
        const gamepads = navigator.getGamepads();
    ...
    renderGameplay() {
        ...
        if (this.sprites.player) {
            this.ctx.drawImage(this.sprites.player, this.state.player.x, this.state.player.y, this.state.player.w, this.state.player.h);
        }
        else {
            this.ctx.fillStyle = '#58a6ff';
