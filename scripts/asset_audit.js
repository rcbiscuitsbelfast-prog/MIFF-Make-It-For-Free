...
function auditRegistry(regPath){
	const out = { errors: [], warnings: [], tiles: [] };
...
function auditPngs(dirList){
	const out = { sheets: [], misaligned: [], oversized: [], missing: [] };
	for (const dir of dirList){
		...
		for (const f of fs.readdirSync(abs)){
			...
			// Heuristic: very large sprite sheets likely need slicing
			if (width >= 2048 || height >= 2048) out.sheets.push({ file: p, width, height });
			// Non power-of-two or non-tile-ish proportions could be misaligned for iso tiles
...
function auditOrchestration(mapPaths, registryIds){
	...
	for (const p of mapPaths){
		...
		const m = readJson(p);
		const layers = (m.layers && m.layers.props && m.layers.props.map) ? m.layers.props.map : [];
		for (const row of layers){
			for (const id of row){ if (id && !registryIds.has(id)) out.errors.push(`${path.basename(p)} references unknown tile: ${id}`); }
		}
		const orch = m.orchestration || {};
		for (const q of (orch.quests||[])){ if (q.tile && !registryIds.has(q.tile)) out.errors.push(`${path.basename(p)} quest uses unknown tile: ${q.tile}`); }
		for (const l of (orch.lore||[])){ if (l.tile && !registryIds.has(l.tile)) out.errors.push(`${path.basename(p)} lore uses unknown tile: ${l.tile}`); }
	}
	return out;
}
...
function generateReport(regOut, pngOut, orchOut){
	...
	lines.push(`Warnings: ${orchOut.warnings.length}`);
	for (const w of orchOut.warnings) lines.push(`- ${w}`);
	lines.push('');
...
(function main(){
	...
	const pngOut = auditPngs(['assets/Isometric Blocks', 'assets/Toppler Medieval']);
