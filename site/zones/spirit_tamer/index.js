...
function showDialogueNode(id){
	...
	if (node.choices){
		node.choices.forEach(ch=>{
			...
			btn.onclick = ()=>{ try{ audio.sfxUI?.play(); }catch{} if (Dialogue[ch.next]?.effect) Dialogue[ch.next].effect(); closeDialogue(); model.state = State.Playing; startReplay(); };
			overlay.appendChild(btn);
		});
	} else if (node.end){
		const btn = document.createElement('button'); btn.className='btn'; btn.textContent = 'Continue';
		btn.onclick = ()=>{ try{ audio.sfxUI?.play(); }catch{} if (node.effect) node.effect(); closeDialogue(); model.state = State.Playing; startReplay(); };
		overlay.appendChild(btn);
	}
}
